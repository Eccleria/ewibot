...
export const onMessageDelete = async (message) => {
  ...
  embed.addFields({name: messageDel.date, value: `${dateStr}`, inline: true}); //date of message creation
  embed.addFields({name: messageDel.channel, value: `<#${message.channelId}>`, inline: true}); //message channel
...
export const onMessageUpdate = async (oldMessage, newMessage) => {
  ...
  if (oMessage.pinned && !nMessage.pinned) {
    ...
    embed.addFields({name: unpinned.title, value: unpinned.text, inline: true}); //add unpinned text
    embed.addFields({name: messageU.channel, value: `<#${oMessage.channelId}>`, inline: true}); //message channel

    //add message link
    const link = `[${messageU.linkMessage}](${nMessage.url})`;
    embed.addFields({name: messageU.linkName, value: link, inline: true});
  ...
  if (!oMessage.pinned && nMessage.pinned) {
    ...
    embed.addFields({name: pinned.title, value: pinned.text, inline: true}); //add unpinned text
    embed.addFields({name: messageU.channel, value: `<#${oMessage.channelId}>`, inline: true}); //message channel
  ...
  if (currentServer.name === "prod") uDate.setHours(uDate.getHours() + 1); //add 1h to date
  const dateStr = uDate.toString().slice(4, 24); //slice date string
  embed.addFields({name: messageU.date, value: `${dateStr}`, inline: true}); //date of message creation
  embed.addFields({name: messageU.channel, value: `<#${oMessage.channelId}>`, inline: true}); //message channel
...
export const onGuildMemberRemove = async (memberKick) => {
  ...
  if (!diff || diff >= 5) {
    ...
    if (textRoles) embed.addFields({name: guildKick.roles, value: textRoles, inline: true}); //add user roles if any
    const messageList = await endCasesEmbed(
      userKick,
      kickLog,
      guildKick,
