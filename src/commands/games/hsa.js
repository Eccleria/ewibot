import { ActionRowBuilder, ButtonStyle, Colors, EmbedBuilder } from "discord.js";
...
export const frontalierAction = async (interaction) => {
  ...
  //check target
  if (!targetUser || targetUser.id === user.id || targetUser.id === client.user.id) {
  ...
  const embedContent = `<@${finalUser.id}>` + ePerso.text + `<@${interaction.user.id}> !`;
