...
const ping = {
  ...
  help: (interaction) => {
    interaction.reply({ content: personality.helloWorld.help, ephemeral: true });
  },
};
...
const help = {
  command: new SlashCommandBuilder()
    ...
    .addStringOption((option) =>
      option
        .setName('command')
        .setDescription("Choix de la commande dont l'aide sera affiché.")
  ...
  action: (interaction) => {
    ...
    const foundCommand = slashCommands.find((cmd) => cmd.command.name === userOption); //find associated command
