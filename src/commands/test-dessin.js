...
const command = new SlashCommandBuilder()
  .setName(PERSONALITY.getCommands().testdessin.name)
  .setDescription(PERSONALITY.getCommands().testdessin.description)
  .setDefaultMemberPermissions(0x0000010000000000); //only moderation staff
...
const action = (interaction) => {
  ...
  const posSizes = getCircleValues(id);
...
const testdessin = {
  ...
  command,
  help: (interaction) => {
    const personality = PERSONALITY.getCommands().testdessin;
